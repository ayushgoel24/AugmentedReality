...
def P3P(Pc, Pw, K=np.eye(3)):
    ...
    for i in range(len(roots)):
        u = ((-1 + (a**2 - c**2) / b**2) * np.square(roots[i]) - 2 * ((a**2 - c**2) / b**2) * cosine_beta * roots[i] + 1 + ((a**2 - c**2) / b**2)) / (2 * (cosine_gamma - roots[i] * cosine_alpha))

        s_1 = np.sqrt((b ** 2) / (1 + roots[i]**2 - 2 * roots[i] * cosine_beta))
